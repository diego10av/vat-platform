...
export async function GET(request: NextRequest) {
  ...
  const rows = await query(
    // Stint 93 — surface primary contact (id + full_name) on every
    // row so the list table can show "who's the contact?" at a
    // glance, mirroring the Opportunities list. crm_matters has had
    // primary_contact_id since migration 032 but no list ever JOIN'd
    // it — fixed here.
