...
export function TaskSignoffCard({ taskId, snapshot, defaultSigner, onChanged }: Props) {
  ...
  return (
    <div className="rounded-md border border-border bg-surface px-4 py-3">
      <h3 className="text-sm font-semibold text-ink mb-2">Sign-off</h3>
      <p className="text-2xs text-ink-muted mb-2">
        Cascading approval. Reviewer requires preparer signed; partner requires reviewer signed.
        Click a signed checkbox to undo (cascades downstream).
      </p>
          ...
          return (
            <div
              ...
              className={[
                ...
                signed ? 'bg-success-50 border-success-200' : locked ? 'bg-surface-alt border-border opacity-60' : 'bg-surface border-border',
              ].join(' ')}
            >
              <button
                type="button"
                ...
                className={[
                  'shrink-0 inline-flex items-center justify-center w-5 h-5 rounded border',
              ...
              >
                {signed && <CheckIcon size={12} />}
              </button>
              <div className="w-[140px] shrink-0 text-sm text-ink font-medium">{label}</div>
              <div className="flex-1 min-w-0">
                {signed ? (
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-ink truncate">{signedName}</span>
                    <span className="text-2xs text-ink-faint">{formatTs(signedAt)}</span>
                  </div>
                ) : (
                  <input
                    value={draftName}
                    ...
                    placeholder={locked ? '—' : 'name to sign as'}
                    disabled={locked}
                    className="w-full px-2 py-1 text-sm border border-border rounded bg-surface disabled:opacity-50"
                  />
                )}
              </div>
            </div>
